...
✅ ハッシュタグは使わない
✅ 絵文字を効果的に使用（2-3個）
✅ URLは23文字でカウントされるため、本文は257文字以内に収める（280-23=257）
...
function generateFallbackTweet({ title = "", contentSnippet = "", category = "news" }) {
  const MAX_LENGTH = 200; // URL分を考慮して余裕を持たせる
  
  // より説明的なフォールバック
  const summary = contentSnippet || title;
  const templates = [
    `🚨注目の動向🚨 ${summary} この変化は業界全体に影響しそう！`,
    `💡興味深い発見💡 ${summary} 今後の展開が気になる🤔`,
    `📈重要なニュース📈 ${summary} チェック必須の内容！`,
  ];
  
  const selectedTemplate = templates[Math.floor(Math.random() * templates.length)];
  return truncateAtSentenceEnd(selectedTemplate, MAX_LENGTH);
}
